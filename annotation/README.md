...
- `<sp>_b1v02` have been used. Scaffolds above 500bp were annotated.
...
``` 
maker_map_ids --prefix DST_ --justify 5 *.max.gff > *.max.map	#example for D.stevensoni.
...
Default parameters against both the `NCBI non-redundant arthropods protein` database. 
